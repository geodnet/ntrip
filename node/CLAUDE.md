...
subproject (`node/`). See the repo root `CLAUDE.md` for how this fits into the rest of the
repository.

## Overview

A minimal, dependency-free Node.js Ntrip client and companion source-table tool for the GEODNET
RTK correction service (see https://github.com/geodnet/GEODNET_RTK_SERVICE). Cross-platform
(Windows/macOS/Linux) — no build system, package manager, or test suite, just plain scripts using
Node's built-in `net` and `fs`.
...
real-time decoded view of each message plus a periodic summary.
